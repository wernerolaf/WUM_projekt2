...
ggplot(dunn, aes( k, y=indeks_Dunna, color = Metoda )) +
...
ggplot(xie_beni, aes( k, y=indeks_Xie_Beni, color = Metoda )) +
    geom_point(aes(y = kmeans, col = "kmeans"))+
    geom_point(aes(y = pam, col = "pam")) + 
    geom_point(aes(y = hclust, col = "hclust"))+
...
ggplot(xie_beni, aes( k, y=indeks_Davies_Bouldin, color = Metoda )) +
    geom_point(aes(y = kmeans, col = "kmeans"))+
    geom_point(aes(y = pam, col = "pam")) + 
    geom_point(aes(y = hclust, col = "hclust"))+
...
Aby zinterpretować powyższe wykresy należy widzieć jak odczytywać wyniki dla danych indeksów. Dla indeksu Dunn'a szukamy maksymalnej wartości, w przypadku Xie Beni  i Davies'a Bouldin'a jest to minimum. Niestety nie widać na nich żadnych zależności. Dla każdej z metod w trzech przyadkach jest różny wskazywany optymalny podział. 
...
Analizując wykres PCA oraz zmeinną "centers" metody kmeans() można zauważyć, że cechy weglug których głównie odbywał się podział to te z przedrostkiem "P4". Są to atrybute odpowiadające pytaniom odnoszącym się do zachowania dziecka na wystawie (radosne, smutne, złe, zaciekawione, znudzone, ruchliwe, skoncentrowane, zrelaksowane).
